...
class Type extends BaseApi
{
    ...
    public function save(Request $request)
    {
        ...
        try{
            ...
            foreach($params['spec'] as $k=>$v){
                ...
                if(!is_array($v['value'])){
                    //如果规格名称的规格值不是数组，将当前整个规格名称信息删除，跳出本次遍历。
                    unset($params['spec'][$k]);
                    continue;
                }
    ...
    {
        //查询模型、模型下规格名称、规格值、属性信息
        $info = \app\common\model\Type::with('specs,attrs,specs.spec_values')->find($id);
        $info = $info ? $info->toArray() : [];
        /*$info = $info->toArray();
        foreach($info['attrs'] as $k=>$v){
            $info['attrs'][$k]['attr_values'] = explode(',', $v['attr_values']);
        }
        unset($v);*/
        $this->ok($info);
    }
